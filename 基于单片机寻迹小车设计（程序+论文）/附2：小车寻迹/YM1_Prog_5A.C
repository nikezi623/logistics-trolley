...
unsigned char 			g_ucRotateStat;								// 转动状态，指示小车是直行还是左、右转
	...
	switch (ucCommand)
	{
		...
		case RUN_ON_LINE:
		{
			...
			else
			{
				...
				g_ucRotateStat = STRAIGHT;												// 初始状态为直行
			}
	...
	if((ucSensorStat&0x06) != 0x06)
	{
		if(g_ucRotateStat == STRAIGHT)
		{
			...
			if((ucSensorStat&0x02) != 0x02)
			{
				...
				g_ucRotateStat = RIGHT_ROTATE;
			}
			else
			{
				// 3号（左侧）传感器不在，左轮置为刹车
				ga_uiMotorCtrl[MOTOR_L] = BRAKE_PWM;
				g_ucRotateStat = LEFT_ROTATE;
			}
